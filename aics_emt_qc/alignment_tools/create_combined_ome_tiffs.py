    ...
    for scene in range(28):
        ...
        for _, _, filenames in os.walk(aligned_dir):
            for filename in [f for f in filenames if f.endswith(".tiff")]:
                if (
                    "argo" not in filename
                    and "aligned_timelapse.ome.tiff" not in filename
                ):
                    block_num = int(
                        filename[
                            filename.find("Block") + 5 : filename.find("Block") + 6
                        ]
                    )
                    scene_num = int(
                        filename[filename.find("Scene") + 6 : filename.find("_aligned")]
                    )
                    if scene_num == scene:
                        if block_num == 7:
                            comb_array[block_num - 1] = AICSImage(
                                os.path.join(aligned_dir, filename)
                            ).data.astype("uint16")
                            print(
                                f"{filename} added: Scene:{scene_num} Block:{block_num}"
                            )
                        elif 0 < block_num < 7:
                            comb_array[block_num - 1, 0:2] = AICSImage(
                                os.path.join(aligned_dir, filename)
                            ).data.astype("uint16")
                            print(
                                f"{filename} added: Scene:{scene_num} Block:{block_num}"
        ...
        OmeTiffWriter.save(
            ...
            aligned_dir
            / "combined_images"
            / f"{os.path.split(aligned_dir.parent)[-1]}_Scene-{scene}_aligned_timelapse.ome.tiff",
        )
